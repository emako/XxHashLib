...
internal static unsafe class XxHash3Impl
{
    ...
    private static ulong HashLen0To16(byte* input, int length, ulong seed)
    {
        if (length > 8)
        ...
        return XXH64_avalanche(seed ^ ((ulong)Secret[56] ^ (ulong)Secret[63]));
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ulong HashLen1To3(byte* input, int length, ulong seed)
    {
        byte c1 = input[0];
        ...
        ulong bitflip = (ReadU32(Secret, 0) ^ ReadU32(Secret, 4)) + seed;
        ulong keyed = combined ^ bitflip;
        return XXH64_avalanche(keyed);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ulong HashLen4To8(byte* input, int length, ulong seed)
    {
        seed ^= (ulong)Swap32((uint)seed) << 32;
        uint input1 = ReadU32(input, 0);
        uint input2 = ReadU32(input, length - 4);
        ulong bitflip = (ReadU64(Secret, 8) ^ ReadU64(Secret, 16)) - seed;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ulong HashLen9To16(byte* input, int length, ulong seed)
    {
        ...
        ulong acc = (ulong)length + Swap64(input_lo) + input_hi + XXH3_mul128_fold64(input_lo, input_hi);
        return XXH3_avalanche(acc);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ulong HashLen17To128(byte* input, int length, ulong seed)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ulong HashLen129To240(byte* input, int length, ulong seed)
    {
        ulong acc = (ulong)length * Prime64_1;
        ...
        acc += XXH3_mix16B(input + length - 16, Secret, 119, seed);
        return XXH3_avalanche(acc);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static ulong HashLong(byte* input, int length, ulong seed)
    {
        ...
        for (int n = 0; n < nbStripes; n++)
        {
            for (int i = 0; i < 16; i += 2)
            {
                ...
                acc += XXH3_mix16B_single(data_val, secret_val ^ (seed + (ulong)(n * 64)));
            }
        }
        ...
        if (remaining > 0)
        {
            ...
            for (int i = 0; i < 16; i += 2)
            {
                ...
                ulong secret_val = ReadU64(Secret, i * 4);
                acc += XXH3_mix16B_single(data_val, secret_val ^ seed);
            }
        }
